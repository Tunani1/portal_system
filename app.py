...
class SignupForm(FlaskForm):
    firstname = StringField('firstname', validators=[InputRequired(), Length(min=4, max=200)])
    lastname = StringField('lastname', validators=[InputRequired(), Length(min=4, max=200)])
...
def signup():
    ...
    if form.validate_on_submit():
        #return '<h1>' + form.username.data + ' ' + form.email.data +  ' ' + form.password.data + ' ' + form.firstname.data + ' ' + form.lastname.data +'</h1>'
        ...
        user_object = Sys_Users.query.filter_by(UserName=username).all()
        if user_object:
            return "Someone else has taken the username"
        
        user = Sys_Users(FirstName=username, LastName=password,UserName=username, UserPassword=password, UserEmail=password)
        db.session(user)
        db.session.commit()
